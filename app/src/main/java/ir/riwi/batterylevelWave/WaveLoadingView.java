...
public class WaveLoadingView extends View {
        ...
        if (mWaveShader != null) {
            ...
            switch (mShapeType) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    Point s = new Point(0, getHeight());
                    Path battery = getBatteryPath(s, getWidth(), getHeight());
                    canvas.drawPath(battery, mWaveBgPaint);
                    canvas.drawPath(battery, mWavePaint);
                    break;
                default:
                    break;
            }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (getShapeType() == 3 || getShapeType() == 4) {
            mCanvasWidth = w;
            mCanvasHeight = h;
        } else {
            mCanvasSize = w;
...
//        mWaveShader = null;
    ...
    private Path getBatteryPath(Point p1, int width, int height) {
        int thirdOfWidth = width / 3;
        int batteryHeadHeight = (int) (thirdOfWidth * 0.66);

        Log.e("p1", p1.toString());
        Path path = new Path();
        path.moveTo(p1.x, p1.y);
        path.lineTo(0, batteryHeadHeight);
        path.lineTo(thirdOfWidth, batteryHeadHeight);
        path.lineTo(thirdOfWidth, 0);
        path.lineTo(2 * thirdOfWidth, 0);
        path.lineTo(2 * thirdOfWidth, batteryHeadHeight);
        path.lineTo(width, batteryHeadHeight);
